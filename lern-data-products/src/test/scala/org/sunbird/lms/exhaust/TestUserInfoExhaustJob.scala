...
class TestUserInfoExhaustJob extends BaseReportSpec with MockFactory with BaseReportsJob {
      ...
      override def dispatch(request: RecordedRequest): MockResponse = {
        ...
        (request.getPath, request.getMethod, requestBody) match {
          ...
          case ("/private/v2/org/preferences/read", "POST", "userPrivateFields") =>
            new MockResponse().setHeader("Content-Type", "application/json").setResponseCode(200).setBody("""{"id":".private.v2.org.preferences.read","ver":"private","ts":"2023-05-26 16:25:42:913+0000","params":{"resmsgid":"976058ce-570a-4c56-a5f9-623141bedd4a","msgid":"976058ce-570a-4c56-a5f9-623141bedd4a","err":null,"status":"SUCCESS","errmsg":null},"responseCode":"OK","result":{"response":{"data": {"piiFields":["courseid","collectionName","batchid","batchName","userid","state","district","orgname","schooludisecode","schoolname","block","cluster","usertype","usersubtype","email", "phone", "username"]}}}}""")
        }
      }
    }
  ...
  def setupRedisData(): Unit = {
    ...
    jedis.hmset("user:user-015", JSONUtils.deserialize[java.util.Map[String, String]]("""{"cluster":"CLUSTER1","firstname":"Manju","subject":"[\"IRCS\"]","schooludisecode":"3183211","usertype":"administrator","usersignintype":"Validated","language":"[\"English\"]","medium":"[\"English\"]","userid":"user-015","schoolname":"DPS, MATHURA","rootorgid":"01250894314817126443","lastname":"D","framework":"[\"sunbird_health\"]","orgname":"Root Org2","phone":"","usersubtype":"deo","district":"bengaluru","grade":"[\"Volunteers\"]","block":"BLOCK1","state":"Karnataka","board":"[\"Sunbird-Health\"]","email":""};"""))
  ...
  it should "generate the user info report with all the users for a batch" in {
    ...
    val strConfig = """{"search":{"type":"none"},"model":"org.sunbird.lms.exhaust.collection.UserInfoExhaustJob","modelParams":{"store":"local","mode":"OnDemand","batchFilters":["TPD"],"searchFilter":{},"sparkElasticsearchConnectionHost":"localhost","sparkRedisConnectionHost":"localhost","sparkUserDbRedisIndex":"0","sparkCassandraConnectionHost":"localhost","sparkUserDbRedisPort":6381,"fromDate":"","toDate":"","storageContainer":"","columnMapping":{"courseid":"Collection Id","collectionName":"Collection Name","batchid":"Batch Id","batchName":"Batch Name","userid":"User UUID","username":"User Name","usertype":"User Type","usersubtype":"User Sub Type","state":"State","district":"District","block":"Block","cluster":"Cluster","orgname":"Org Name","email":"Email ID","phone":"Mobile Number","consentflag":"Consent Provided","consentprovideddate":"Consent Provided Date","schooludisecode":"School Id","schoolname":"School Name"},"csvColumns":["courseid", "collectionName", "batchid", "batchName", "userid", "username", "state", "district", "orgname", "email", "phone","consentflag", "block", "cluster", "usertype", "usersubtype", "schooludisecode", "schoolname","consentprovideddate"]},"parallelization":8,"appName":"UserInfo Exhaust"}""".stripMargin
  ...
  it should "generate the user info report with all the users for a batch with requested_channel as System" in {
    ...
    implicit val fc = new FrameworkContext()
    val strConfig = """{"search":{"type":"none"},"model":"org.sunbird.lms.exhaust.collection.UserInfoExhaustJob","modelParams":{"store":"local","mode":"OnDemand","batchFilters":["TPD"],"searchFilter":{},"sparkElasticsearchConnectionHost":"localhost","sparkRedisConnectionHost":"localhost","sparkUserDbRedisIndex":"12","sparkCassandraConnectionHost":"localhost","fromDate":"","toDate":"","storageContainer":"","columnMapping":{"courseid":"Collection Id","collectionName":"Collection Name","batchid":"Batch Id","batchName":"Batch Name","userid":"User UUID","username":"User Name","usertype":"User Type","usersubtype":"User Sub Type","state":"State","district":"District","block":"Block","cluster":"Cluster","orgname":"Org Name","email":"Email ID","phone":"Mobile Number","consentflag":"Consent Provided","consentprovideddate":"Consent Provided Date","schooludisecode":"School Id","schoolname":"School Name"},"csvColumns":["courseid", "collectionName", "batchid", "batchName", "userid", "username", "state", "district", "orgname", "email", "phone","consentflag", "consentprovideddate", "block", "cluster", "usertype", "usersubtype", "schooludisecode", "schoolname"]},"parallelization":8,"appName":"UserInfo Exhaust"}"""
    val jobConfig = JSONUtils.deserialize[JobConfig](strConfig)
    implicit val config = jobConfig
  ...
  it should "generate the report without modelParams present" in {
    ...
    implicit val fc = new FrameworkContext()
    val strConfig = """{"search":{"type":"none"},"model":"org.sunbird.lms.exhaust.collection.UserInfoExhaustJob","parallelization":8,"appName":"UserInfo Exhaust"}"""
    val jobConfig = JSONUtils.deserialize[JobConfig](strConfig)
    implicit val config = jobConfig
    ...
    try {
      ...
      case ex: NoSuchElementException => Console.println(s"NoSuchElementException occured")
    }
...
//    user001.map {f => f.`Collection Name`}.head should be ("ADOPT_BOOK")
...
//    user009.map {f => f.`Collection Name`}.head should be ("ADOPT_BOOK")
...
//      pResponse.getString("err_message") should be ("")
//      pResponse.getString("dt_job_submitted") should be ("2020-10-19 05:58:18.666")
//      pResponse.getString("download_urls") should be (s"{userinfo-exhaust/$requestId/batch-003_userinfo_${getDate()}.zip}")
//      pResponse.getString("dt_file_created") should be (null)
//      pResponse.getString("iteration") should be ("0")
//    }
//
...
//  }
  ...
  /**
    ...
    */
  it should "not generate the user info report excluding the user who have not provided consent" in {
    EmbeddedPostgresql.execute(s"TRUNCATE $jobRequestTable")
