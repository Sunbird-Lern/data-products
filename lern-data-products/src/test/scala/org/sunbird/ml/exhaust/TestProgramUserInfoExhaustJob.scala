...
class TestProgramUserInfoExhaustJoB extends BaseReportSpec with MockFactory with BaseReportsJob {
  ...
  it should "Mark duplicate requests and Insert Status as SUCCESS" in {
    ...
    val requestId1 = "37564AN8F134RR7532F125651B51S17D"

    ProgramUserInfoExhaustJob.execute()

    val pResponse1 = EmbeddedPostgresql.executeQuery("SELECT * FROM job_request WHERE job_id='program-user-exhaust' and request_id='37564AN8F134RR7532F125651B51S17D'")

    while (pResponse1.next()) {
      pResponse1.getString("status") should be("SUCCESS")
      pResponse1.getString("err_message") should be("")
      pResponse1.getString("dt_job_submitted") should be("2023-01-25 05:58:18.666")
    }

    val pResponse2 = EmbeddedPostgresql.executeQuery("SELECT * FROM job_request WHERE job_id='program-user-exhaust' and request_id='37564AN8F134RR7532F125651B51S17F'")

    while (pResponse2.next()) {
      pResponse2.getString("status") should be("SUCCESS")
      pResponse2.getString("err_message") should be("")
